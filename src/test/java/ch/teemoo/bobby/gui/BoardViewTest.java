...
public class BoardViewTest {
	...
	public void display_onlyRook_boardHasCorrectSetup(boolean reversed, String letter) {
		...
		Supplier<Stream<SideLabel>> supplier = () -> Arrays.stream(view.getContentPane().getComponents())
				.filter(SideLabel.class::isInstance).map(it -> (SideLabel) it).filter(it -> !it.getText().isEmpty());

		assertThat(supplier.get()).hasSize(8 * 4);
		assertThat(supplier.get().filter(it -> it.getText().charAt(0) >= 'a' && it.getText().charAt(0) <= 'h'))
				.hasSize(8 * 2);
		assertThat(supplier.get().filter(it -> it.getText().charAt(0) >= '1' && it.getText().charAt(0) <= '8'))
				.hasSize(8 * 2);
		assertThat(Arrays.stream(view.getContentPane().getComponents()).filter(Square.class::isInstance)).hasSize(64);
	}
