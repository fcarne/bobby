...
public class Tournament {
  ...
  public Map<Player, Float> getParticipantScores() {
    ...
    for (Player player : participants) {
      scoresMap.put(player,
          (float) (matches.stream()
              .filter(m -> m.isPlayerTakingPartToTheMatch(player))
              .mapToDouble(m -> m.getScoreByPlayer(player))
              .sum()));
    }
    return scoresMap;
  }
  ...
  public String getScoreboard() {
    List<Map.Entry<Player, Float>> entries = getParticipantScores().entrySet()
        .stream()
